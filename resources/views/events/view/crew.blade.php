...
<p class="crew-count">
    @if($event->isSocial())
        {{ $event->countCrew('members') }} members and {{ $event->countCrew('guests') }} guests ({{ $event->countCrew('confirmed') }} paid)
    @elseif($event->isTraining())
        {{ $event->countCrew('em') }} instructors and {{ ($event->countCrew('members') - $event->countCrew('em')) }} attendees
        ...
        <div class="btn-group">
            ...
            @else
                <button class="btn btn-success"
                        data-submit-ajax="{{ route('event.volunteer', ['id' => $event->id]) }}"
                        @if($event->isSocial())
                        data-submit-confirm="Are you sure you want to sign-up for this social? By signing up you agree to pay any costs, even if you can no longer attend."
                        @endif
                        data-redirect="true"
                        data-redirect-location="{{ route('event.view', ['id' => $event->id, 'tab' => 'crew']) }}"
                        type="button">
