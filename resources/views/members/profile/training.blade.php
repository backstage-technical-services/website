        ...
        <ul class="nav nav-pills nav-stacked category-list" role="tablist">
            @foreach ($SkillCategories as $i => $category)
                <li{{ $i == 0 ? ' class=active' : '' }}>
                    ...
                    <span class="label label-default">{{ $user->countSkills($category->id ? $category : null) }} /
                        {{ count($category->skills) }}</span>
                    </li>
            @endforeach
        </ul>
    </div>
                    ...
                    <tbody>
                        @forelse($category->skills as $skill)
                            <tr>
                                <td class="skill-name">
                                    <a class="grey"
                                        href="{{ route('training.skill.view', ['id' => $skill->id]) }}">{{ $skill->name }}</a>
                                </td>
                        ...
                        @empty
                            <tr>
                                <td colspan="3"><em>&ndash; there aren't any skills in this category &ndash;</em>
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
