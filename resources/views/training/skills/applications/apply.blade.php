...
@section('content')
    ...
    <p>
        If you feel like you should be awarded a skill, or be a higher level than currently awarded, you can use this form
        to apply for your new skill level. This will then be reviewed by the Training and Safety Officer.
    </p>
        ...
        <select class="form-control" name="level">
            @for ($i = 1; $i <= 3; $i++)
                @if (!$skill || $skill->isLevelAvailable($i))
                    <option value="{{ $i }}"{{ !in_array($i, $AvailableLevels) ? ' disabled' : '' }}>
                        {{ $LevelNames[$i] }}
                    </option>
                @endif
            @endfor
    ...
    <div class="form-group @InputClass('reasoning')">
        ...
        {!! Form::textarea('reasoning', null, [
            'class' => 'form-control',
            'placeholder' => 'Explain why you should be awarded this level - use examples to help justify your application',
            'rows' => 6,
        ]) !!}
