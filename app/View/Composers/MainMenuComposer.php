...
class MainMenuComposer implements ViewComposer
{
    ...
    {
        $eventsMenu = $menu->add('Events Diary', route('event.diary'))
                           ->active('events/*')
                           ->attr('class', 'dropdown');
        
        if ($this->isMember) {
            $eventsMenu->add('Submit event report', route('event.report'))
    ...
    {
        $membersMenu = $menu->add('Members\' Area', route('auth.login'))
                            ->active('members/*')
                            ->attr('class', 'dropdown');
        
        if ($this->isRegistered) {
            $profileMenu = $membersMenu->add('My Profile', route('member.profile'))
                                       ->attr('class', 'dropdown profile');
            ...
            if ($this->isMember) {
                ...
                // Users
                $usersMenu = $membersMenu->add('The Membership', route('membership.view'))
                ...
                $miscMenu = $membersMenu->add('Other', '#')
                                        ->attr('class', 'dropdown misc')
                                        ->divide();
                if ($this->isMember) {
                    $miscMenu->add('Committee elections', route('election.index'))
                             ->active('elections/*');
                ...
                $membersMenu->add('Report an Accident', route('contact.accident'))
                            ->divide();
            }
    ...
    {
        $resourcesMenu = $menu->add('Resources', route('resource.search'))
                              ->active('resources/*')
                              ->attr('class', 'dropdown');
        
        if ($this->isRegistered) {
            $resourcesMenu->add('Event Reports', route('resource.search', ['category' => 'event-reports']));
            $resourcesMenu->add('Event Risk Assessments',
